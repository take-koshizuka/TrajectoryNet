...
def integrate_backwards(end_samples, model_g, model_f, savedir, ntimes=100, memory=0.1, device='cpu'):
    ...
    with torch.no_grad():
        ...
        for i, itp in enumerate(int_tps[::-1][:-1]):
            ...
            x = model_g(ys[-1], reverse=False)
            next_x, delta_logp = cnf(x, zero, integration_times=integration_times)
        ...
        ys = torch.stack(ys, 0)
        ys = ys.cpu().numpy()
        np.save(os.path.join(savedir, 'backward_trajectories.npy'), ys)
...
def main(args):
    ...
    #integrate_backwards(end_time_data, model_g, model_f, args.save, ntimes=100, device=device)

    losses_list = []
    #for factor in np.linspace(0.05, 0.95, 19):
    #for factor in np.linspace(0.91, 0.99, 9):
    if cfg['dataset']['name'] == 'CHAFFER':   # Do timepoint adjustment
        print('adjusting_timepoints')
        lt = cfg['leaveout_timepoint']
        if lt == 1:
            ...
            raise RuntimeError('Unknown timepoint %d' % cfg['leaveout_timepoint'])
        cfg['int_tps'][lt] = (1 - factor) * cfg['int_tps'][lt-1] + factor * cfg['int_tps'][lt+1]

    losses = eval_utils.evaluate_kantorovich_v2(device, cfg, data, model_g, model_f)
    ...
    ys, emd = eval_utils.evaluate_kantorovich(device, cfg, data, model_g, model_f)
    print(emd)
