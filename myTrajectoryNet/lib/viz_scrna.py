...
def save_vectors(prior_logdensity, model_g, model_f, data_samples, full_data, labels, savedir, skip_first=False, ntimes=101, end_times=None, memory=0.01, device='cpu', lim=4):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for cnf in model_f.chain:
            ...
            # Integrate over evenly spaced samples
            x_traj, logpx = cnf(z_samples, logp_samples, integration_times=integration_list[0], reverse=True)
            y_traj, logpy = model_g(x_traj, logpx, reverse=True)

            full_traj_x = [(x_traj, logpx)]
            full_traj_y = [(y_traj, logpy)]
            for int_times in integration_list[1:]:
                prev_x, prev_logp = full_traj_x[-1]
                x_traj, logpx = cnf(prev_x[-1], prev_logp[-1], integration_times=int_times, reverse=True)
                y_traj, logpy = model_g(x_traj, logpx, reverse=True)
                full_traj_x.append((x_traj, logpx))
                full_traj_y.append((y_traj, logpy))
        
            full_zip = list(zip(*full_traj_y))
            y_traj = torch.cat(full_zip[0], 0)
            y_traj = y_traj.cpu().numpy()
            ...
            y_traj = np.swapaxes(y_traj, 0, 1)
            if skip_first:
                y_traj = y_traj[:, ntimes:, :]
            ax.scatter(y_traj[:,0,0], y_traj[:,0,1], s=20, c='k')
            for yk in y_traj:
            #for zk in z_traj[:,ntimes:,:]:
                ax.scatter(yk[:,0], yk[:,1], s=1, c = np.linspace(0,1,yk.shape[0]), cmap='Spectral')
            ax.set_xlim(-lim, lim)
            ax.set_ylim(-lim, lim)
            #ax.set_ylim(4, -4)
            makedirs(savedir)
...
def save_trajectory_density(prior_logdensity, model_g, model_f, data_samples, savedir, ntimes=101, end_times=None, memory=0.01, device='cpu'):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for cnf in model_f.chain:
            ...
            grid_y_traj, grid_logpy_traj = [], []
            inds = torch.arange(0, z_grid.shape[0]).to(torch.int64)
            for ii in torch.split(inds, int(z_grid.shape[0] * memory)):
                _grid_x_traj, _grid_logpx_traj = cnf(
                    z_grid[ii], logp_grid[ii], integration_times=integration_list[0], reverse=True
                )
                _grid_y_traj, _grid_logpy_traj = model_g(_grid_x_traj, _grid_logpx_traj, reverse=True)

                full_traj_x = [(_grid_x_traj, _grid_logpx_traj)]
                full_traj_y = [(_grid_y_traj, _grid_logpy_traj)]

                for int_times in integration_list[1:]:
                    prev_x, prev_logpx = full_traj_x[-1]
                    _grid_x_traj, _grid_logpx_traj = cnf(
                        prev_x[-1], prev_logpx[-1], integration_times=int_times, reverse=True
                    )
                    _grid_y_traj, _grid_logpy_traj = model_g(_grid_x_traj, _grid_logpx_traj, reverse=True)

                    full_traj_x.append((_grid_x_traj, _grid_logpx_traj))
                    full_traj_y.append((_grid_y_traj, _grid_logpy_traj))

                full_zip = list(zip(*full_traj_y))
                _grid_y_traj = torch.cat(full_zip[0], 0).cpu().numpy()
                _grid_logpy_traj = torch.cat(full_zip[1], 0).cpu().numpy()
                print(_grid_y_traj.shape)
                grid_y_traj.append(_grid_y_traj)
                grid_logpy_traj.append(_grid_logpy_traj)
                
            grid_y_traj = np.concatenate(grid_y_traj, axis=1)[ntimes:]
            grid_logpy_traj = np.concatenate(grid_logpy_traj, axis=1)[ntimes:]

            #plt.figure(figsize=(8, 8))
            #fig, axes = plt.subplots(2,1, gridspec_kw={'height_ratios': [7, 1]}, figsize=(5,7))
            for _ in range(grid_y_traj.shape[0]):
                fig, axes = plt.subplots(2,1, gridspec_kw={'height_ratios': [7, 1]}, figsize=(8,8))
                #plt.clf()
                ax = axes[0]
                # Density
                y, logqy = grid_y_traj[t], grid_logpy_traj[t]

                xx = y[:, 0].reshape(npts, npts)
                yy = y[:, 1].reshape(npts, npts)
                qz = np.exp(logqy).reshape(npts, npts)
                ...
                ax.axvline(t / grid_y_traj.shape[0], c='k', linewidth=15)
                ax.set_title('Time')
...
def save_trajectory(prior_logdensity, prior_sampler, model_g, model_f, data_samples, savedir, ntimes=101, end_times=None, memory=0.01, device='cpu'):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for cnf in model_f.chain:
            ...
            grid_y_traj = np.concatenate(grid_y_traj, axis=1)
            grid_logpy_traj = np.concatenate(grid_logpy_traj, axis=1)

            plt.figure(figsize=(8, 8))
            for _ in range(y_traj.shape[0]):
                ...
                yk = y_traj[t]
                ax.hist2d(yk[:, 0], yk[:, 1], range=[[-4, 4], [-4, 4]], bins=200)
                ax.invert_yaxis()
                ax.get_xaxis().set_ticks([])
                ax.get_yaxis().set_ticks([])
