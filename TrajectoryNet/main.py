...
def compute_loss(device, args, model, growth_model, logger, full_data):
    ...
    if args.top_k_reg > 0:
        ...
        if args.leaveout_timepoint not in tp_z_map:
            ...
            idxs = np.logical_or((args.data.get_times() == (args.leaveout_timepoint-1) ), (args.data.get_times() == (args.leaveout_timepoint+1)))
            local_data = (
                torch.from_numpy(
                    args.data.get_data()[idxs]
                )
                .type(torch.float32)
                .to(device)
            )
        ...
        cdist = torch.cdist(samples_05, local_data)
